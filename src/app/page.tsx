      ...
      <header className="flex h-16 shrink-0 items-center gap-4 border-b px-4 md:px-8 bg-white/70 backdrop-blur-md sticky top-0 z-50">
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="ghost" size="icon" className="hover:bg-primary/5 shrink-0">
        ...
        <div className="flex items-center gap-2 overflow-hidden mr-auto">
          <Calculator className="h-6 w-6 text-primary shrink-0 hidden sm:block" />
          <h2 className="text-xl font-bold font-headline text-primary tracking-tight truncate hidden lg:block">My Apex Calc</h2>
        </div>
        ...
        <div className="flex-1 max-w-xl relative">
          <div className="relative">
            ...
            <Input 
              placeholder="Search 40+ precision calculators..." 
              className="pl-10 pr-10 rounded-full bg-muted/50 border-none focus-visible:ring-primary h-10 w-full"
              value={searchQuery}
        ...
        <nav className="hidden md:flex items-center gap-6 shrink-0 ml-4">
            ...
            <section className="space-y-8 min-h-[60vh]">
              <div className="flex items-center justify-between border-b pb-4">
                <div className="space-y-1">
                  <h2 className="text-2xl md:text-3xl font-bold text-primary">Search Results</h2>
                  <p className="text-sm text-muted-foreground font-medium">Found {searchResults.length} tools matching "{searchQuery}"</p>
                </div>
              </div>
              ...
              {searchResults.length > 0 ? (
                <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {searchResults.map((calc) => (
                    <li key={calc.id} className="animate-fade-in">
                      <Link 
                        href={calc.path}
                ...
                <div className="text-center py-20 bg-muted/20 rounded-[2.5rem] border-2 border-dashed">
                  ...
                  <h3 className="text-lg font-bold text-muted-foreground">No tools found</h3>
                  <p className="text-sm text-muted-foreground max-w-xs mx-auto mt-1">We couldn't find any calculator matching your search. Try checking your spelling or using different keywords.</p>
                  <Button variant="outline" className="mt-6 rounded-xl" onClick={() => setSearchQuery('')}>Clear Search</Button>
                </div>
              )}
            </section>
          ) : (
          ...
          <div className="pt-12 text-center">
            <div className="inline-flex flex-col items-center gap-4 p-8 bg-muted/30 rounded-[2rem] border border-dashed border-muted-foreground/20 w-full">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground max-w-md mx-auto">Access our full list of over 40+ professional-grade calculators in the centralized dashboard.</p>
              </div>
              <Link 
                href="/all-calculators" 
              ...
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-black opacity-60">My Apex Calc v1.1</p>
            </div>
          </div>
        </div>
      </main>
